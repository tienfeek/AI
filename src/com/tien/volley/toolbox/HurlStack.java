...
import com.tien.volley.Request.Method;
...
public class HurlStack implements HttpStack {
	...
	public interface UrlRewriter {
		...
		/**
		 * Returns a URL to use instead of the provided one, or null to indicate this URL should not be used
		 * at all.
		 */
...
//		SharedPreferences sp = SKApplication.getInstance().getSharedPreferences("cookie", Context.MODE_PRIVATE);
//		String cookie = sp.getString("cookie", "");
//		String[] cookies = cookie.split(":");
//		for (String cookieUnit : cookies) {
//			connection.addRequestProperty("Cookie", cookieUnit);
//		}
		
		connection.addRequestProperty("http.protocol.handle-redirects", "false");
		...
		int responseCode = connection.getResponseCode();
		  XLog.i("wanges", "responseCode:"+responseCode);
		if (responseCode == -1) {
...
//          List<String> reponseCookies = connection.getHeaderFields().get("Set-Cookie");
//          XLog.i("wanges", "reponseCookies:"+reponseCookies);
//      }
		...
		for (Entry<String, List<String>> header : connection.getHeaderFields().entrySet()) {
			if (header.getKey() != null) {
			    ...
			    for(int i=0; i<values.size(); i++){
			        Header h = new BasicHeader(header.getKey(), values.get(i));
...
//		return HttpUtil.createConnection(url);
